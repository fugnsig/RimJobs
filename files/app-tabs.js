...
Object.assign(App, {
    ...
    c.innerHTML = `
      <div class="settings-card">
        ...
        <div class="settings-row">
          ...
          <div style="display:flex; gap:8px">
            <button class="btn ${s.theme==='dark'?'btn-primary':''}" onclick="App.state.settings.theme='dark'; App.applyTheme(); App.renderSettings()">Dark</button>
            <button class="btn ${s.theme==='light'?'btn-primary':''}" onclick="App.state.settings.theme='light'; App.applyTheme(); App.renderSettings()">Light</button>
          </div>
        </div>
      </div>
      ...
      <div class="settings-card">
        ...
        <div style="margin-top:10px">
          <div class="settings-label" style="margin-bottom:6px">Active mods</div>
  ...
  renderIdeology() {
    ...
    // Calculate aggregate effects from selected precepts. Choices that are no longer
    // valid under the current memes (their enabling meme was removed, or a blocker
    // was added) are skipped, so the totals can never include orphaned options.
    const preceptEffects = { mood: 0, workSpeed: 0, combatSkill: 0, socialSkill: 0, researchSpeed: 0 };
    Object.entries(ideo.precepts).forEach(([pId, optId]) => {
      const pDef = IDEO_PRECEPT_DEFS.find(x => x.id === pId);
      if (!pDef) return;
      if (!this._ideoOptionValid(pDef, optId, ideo.memes)) return;
      const opt = pDef.options.find(o => o.id === optId);
      if (!opt) return;
      if (opt.mood) preceptEffects.mood += opt.mood;
      if (opt.workSpeed) preceptEffects.workSpeed += opt.workSpeed;
      if (opt.combatSkill) preceptEffects.combatSkill += opt.combatSkill;
      if (opt.socialSkill) preceptEffects.socialSkill += opt.socialSkill;
      if (opt.researchSpeed) preceptEffects.researchSpeed += opt.researchSpeed;
    });

    // Combined totals
    const totalMood = memeEffects.mood + preceptEffects.mood;
    const totalWorkSpeed = memeEffects.workSpeed + preceptEffects.workSpeed;
    const totalCombat = memeEffects.combatSkill + preceptEffects.combatSkill;
    const totalSocial = memeEffects.socialSkill + preceptEffects.socialSkill;
    const totalResearch = memeEffects.researchSpeed + preceptEffects.researchSpeed;
      ...
      <div style="width:100%">
        <div class="view-header">
          ...
          <div class="view-header-actions" style="display:flex; gap:8px; align-items:center; ${isWidget ? 'flex-wrap:wrap; width:100%' : ''}">
            ...
            <button class="btn btn-sm" onclick="App.scanIdeologyMods()" title="Scan your RimWorld install and mod folder for modded memes and rituals, then add them to the planner. Cache-backed, so repeat scans are fast." style="font-size:var(--f-xs); padding:4px 10px">Scan Ideology Mods</button>
        ...
        <div style="display:grid; grid-template-columns:${isWidget ? '1fr' : '1fr 1fr'}; gap:var(--gap-lg); margin-bottom:var(--gap-lg)">
          <!-- MEMES -->
          <div class="settings-card">
            <div class="section-title section-title--sm">Memes <span style="font-size:var(--f-xs); color:var(--text3); font-weight:400">(1 structure + up to 3 memes${ideo.type === 'fluid' ? ', fluid starts with 1 meme' : ''})</span></div>
            ${(() => {
        ...
        <!-- PRECEPTS -->
        <div class="settings-card" style="margin-bottom:var(--gap-lg)">
          <div class="section-title section-title--sm">Precepts <span style="font-size:var(--f-xs); font-weight:400; color:var(--accent); opacity:0.8">ACTIVE</span></div>
          <div style="display:grid; grid-template-columns:${isWidget ? '1fr' : 'repeat(auto-fill, minmax(280px, 1fr))'}; gap:12px">
            ${IDEO_PRECEPT_DEFS.map(p => {
              // Check if any meme forces this precept
              let forcedValue = null;
              let forcedByMeme = '';
              ideo.memes.forEach(mId => {
                const m = IDEO_MEMES.find(x => x.id === mId);
                if (m && m.forcedPrecepts && m.forcedPrecepts[p.id]) {
                  forcedValue = m.forcedPrecepts[p.id];
                  forcedByMeme = m.label;
                }
              });
              // A stored choice that is no longer valid under the current memes shows
              // as Not Set (and is excluded from the totals above) instead of lingering.
              const storedValid = ideo.precepts[p.id] && this._ideoOptionValid(p, ideo.precepts[p.id], ideo.memes);
              const selected = forcedValue || (storedValid ? ideo.precepts[p.id] : '') || '';
              const selectedOpt = p.options.find(o => o.id === selected);

              // Filter options based on meme requirements
              const availableOptions = p.options.filter(o => {
                if (o.blockedByMemes && o.blockedByMemes.some(m => ideo.memes.includes(m))) return false;
                if (o.requiredMemes && !o.requiredMemes.some(m => ideo.memes.includes(m))) return false;
                if (o.enabledByMemes && !o.enabledByMemes.some(m => ideo.memes.includes(m))) return false;
                return true;
              });
              ...
              return `<div style="background:var(--surface2); padding:12px; border-radius:8px; border-left:3px solid ${selectedOpt && selectedOpt.mood > 0 ? 'var(--ok-txt)' : selectedOpt && selectedOpt.mood < 0 ? 'var(--p4-txt)' : 'var(--border-med)'}; ${forcedValue ? 'opacity:0.8' : ''}">
                ...
                ` : `
                  <select class="skill-input" style="width:100%; font-size:var(--f-xs)" onchange="App.setIdeoPrecept('${p.id}', this.value)">
        ...
        <!-- RITUALS -->
        <div class="settings-card" style="margin-bottom:var(--gap-lg)">
          <div class="section-title section-title--sm">Rituals <span style="font-size:calc(9px * var(--font-scale)); color:var(--accent); opacity:0.7; margin-left:6px">REFERENCE</span></div>
          ...
          <div style="display:flex; flex-wrap:wrap; gap:6px">
            ${this._allRituals().map(r => {
              ...
              return `<button class="btn btn-sm ${active ? 'btn-accent' : ''}"
                ...
                onclick="App.toggleIdeoRitual('${r.id}')"
  ...
  renderHelp() {
      ...
      <div class="help-layout" id="helpLayout">
        ...
        <div class="settings-card" style="max-width:760px; margin:0 auto">
          <div style="display:flex; align-items:center; gap:10px; margin-bottom:${bodyGap}">
            ...
            <h2 class="view-title" style="font-size:${hdrSize}">Colony Planner Manual</h2>
          </div>
          ...
          <div class="help-body" style="display:flex; flex-direction:column; gap:${bodyGap}; font-size:${bodySize}; color:var(--text2); line-height:1.7">
            ...
            Set each pawn's job priorities from 1 (highest) to 4 (lowest). Empty cells mean the job is disabled. Pawn names are editable directly in the table, changes sync across all views.
            <div style="margin-top:4px; padding-left:12px; border-left:2px solid var(--accent-glow)">
              • <strong>Left Click:</strong> Increase Priority (1 → 2 → 3 → 4 → None)<br>
              • <strong>Right Click:</strong> Decrease Priority (None → 4 → 3 → 2 → 1)<br>
              • <strong>Mouse Wheel:</strong> Rapidly cycle through levels<br>
              • <strong>Keys 1-4:</strong> Set priority level directly<br>
              • <strong>Keys 0 or Del:</strong> Clear the priority
            </div>
            The <strong>Auto-Assign</strong> button uses a dynamic ranking algorithm to ensure 100% job coverage while prioritising specialists and respecting mood modes. <em>It weighs skills, passions, role and xenotype/gene effects, including disabled work. Vanilla xenotype strengths and weaknesses are fully accounted for, and modded ones too after a Scan Mods (a few C#-driven mod effects still can't be read). Children only receive jobs their age allows (hauling/cleaning at 3, most work at 7, skilled work at 10-13, matching the game's age gates), and pawns who were downed in the imported save get nothing until they recover - click the Downed chip on their card to clear it manually.</em> The <strong>Optimiser</strong> button runs a colony-wide analysis to surface gaps and recommend better assignments (see Pawn Optimiser below).</div>
            ...
            <div style="margin-top:4px; padding-left:12px; border-left:2px solid var(--accent-glow)">
              ...
              • <strong>Xenotype &amp; Role:</strong> Dropdowns apply stat modifiers, gene effects, locked incapabilities, and work-speed bonuses.<br>
              ...
              • <strong>Health:</strong> Read-only health summary imported from save files. Shows missing parts (○), prosthetics (◆), implants (◇), injuries (●), and conditions (+) with resolved body part names. Redundant entries are collapsed -e.g. a missing hand won't also list every missing finger, and a prosthetic suppresses the "missing" label for that part.<br>
            ...
            Manage vanilla and custom xenotypes via Settings &gt; Xenotype Manager. Custom xenotypes can be fully configured:
            <div style="margin-top:4px; padding-left:12px; border-left:2px solid var(--accent-glow)">
            ...
            Create and edit custom traits via Settings &gt; Trait Manager. Custom traits support full post-creation editing:
            <div style="margin-top:4px; padding-left:12px; border-left:2px solid var(--accent-glow)">
            ...
            <div style="margin-top:4px; padding-left:12px; border-left:2px solid var(--accent-glow)">
              ...
              • <strong>Threat Presets:</strong> Simulate DPS against specific armour targets (Unarmoured, Flak, Centipede, Marine, Cataphract).<br>
            ...
            <div><strong style="color:var(--text)">Apparel &amp; Comparison</strong> <span style="font-size:var(--f-xs); color:var(--text3)">(Armoury sub-tabs)</span><br>
            ...
            A high-level dashboard showing your colony's Survival Index, a Radar Chart of average skills, and a list of labour bottlenecks (missing critical workers). The radar chart is responsive and scales with window size - hover any axis to highlight it and read that skill's exact colony average.</div>

            <div><strong style="color:var(--text)">Ideology</strong><br>
            Plan your colony's belief system before committing in-game. Choose Fixed (all memes upfront) or Fluid (start with 1, reform later). Pick 1 Structure plus up to 3 Memes, configure Precepts, plan Specialist Roles, and pick Rituals.
            <div style="margin-top:4px; padding-left:12px; border-left:2px solid var(--accent-glow)">
              ...
              • <strong>Precepts:</strong> 37 configurable precept groups across Violence, Social, Food, Lifestyle, Death, and Work categories. Each has multiple severity levels with mood and stat effects. Choices that stop being valid when you change memes show as Not Set and stop counting in the totals.<br>
              ...
              • <strong>Colony Impact:</strong> Live summary showing aggregate Mood, Work Speed, Combat, and Research effects from your meme + precept selections. These modifiers feed directly into viability scores, effective skills, and work speed calculations.<br>
              ...
              • <strong>Save File Ideologies:</strong> After importing a .rws save, the <strong>Save File Ideologies</strong> button (next to Reset) lists every ideology in that save in full - name, deities, memes, roles, and precepts (grouped by issue). A pill showing your colony's own ideology loads it straight into the planner. Both are greyed out until a save is imported.<br>
              • <strong>Scan Ideology Mods:</strong> Scans your install and mod folder for modded <em>memes</em> (including modded structures, which keep the pick-one rule, with conflicts read from their exclusion tags and impact from the def) and modded <em>rituals</em>, then adds them to the planner with a Mod tag. Cache-backed, so repeat scans are fast.
            </div></div>
            ...
            <div style="margin-top:4px; padding-left:12px; border-left:2px solid var(--accent-glow)">
              • <strong>Import-only:</strong> Records exist only inside a RimWorld save, so the tab shows a prompt until you import one (the .rws button in the sidebar). Manually-built colonies won't have records.<br>
            ...
            <div><strong style="color:var(--text)">Pawn Optimiser</strong><br>
            Found in the Priorities tab (click the brain icon). Analyses your colony's current job assignments and flags problems:
            <div style="margin-top:4px; padding-left:12px; border-left:2px solid var(--accent-glow)">
            ...
            Import colonists directly from a RimWorld save file. Click the <strong>.rws</strong> button in the sidebar pawn actions.
            <div style="margin-top:4px; padding-left:12px; border-left:2px solid var(--accent-glow)">
              ...
              • <strong>Refresh Save:</strong> After importing, a refresh button appears next to Import Game Save. Save your game in RimWorld, then click refresh to pull in updated skills, new injuries, recruited pawns, and other changes without re-picking the file. Existing pawns are matched by ID and updated in-place - your custom roles, traits, and mood presets are preserved.
            </div></div>
            ...
            <div style="margin-top:4px; padding-left:12px; border-left:2px solid var(--accent-glow)">
              • <strong>General:</strong> Light/dark theme, font sizes, accent colour, and grid coordinates toggle.<br>
              • <strong>Behaviour:</strong> Startup window mode, minimise-to-tray, tooltips, deletion confirmation, item limits, and "Pre-load mod data in the background" (warms the mod scan cache after launch so the Pawn Manager, Armoury, and Comparison open instantly).<br>
              • <strong>Priorities Input:</strong> How the priority table responds to the mouse - click direction, wheel direction, wheel on/off, and priority wrapping.<br>
              ...
              • <strong>Xenotype Manager:</strong> Create and edit custom xenotypes with skill mods, genes, incapable tags, and mod source labels.<br>
              • <strong>Trait Manager:</strong> Create and edit custom traits with work speed, learning rate, break threshold, and skill modifiers.<br>
              • <strong>Custom Jobs:</strong> Add modded work types and assign them to skill categories.<br>
              • <strong>Mod Awareness:</strong> See active mod sources and toggle hiding modded content globally.<br>
              • <strong>Data:</strong> Export/import your full colony data as JSON for backup or sharing.
            </div></div>

            <div><strong style="color:var(--text)">Formulas &amp; Calculations</strong><br>
            All pawn stats are derived from multiple stacking sources. Here are the exact formulas used:
            <details style="margin-top:8px; border:1px solid var(--border); border-radius:8px; padding:0; overflow:hidden">
              <summary style="cursor:pointer; padding:8px 12px; font-size:var(--f-xs); font-weight:700; color:var(--accent); background:var(--surface2); user-select:none">Show formulas</summary>
              <div style="padding:10px 12px; font-size:var(--f-xs); color:var(--text3); line-height:1.8; font-family:monospace; background:var(--surface3)">
            ...
            <div style="margin-top:4px; padding-left:12px; border-left:2px solid var(--accent-glow)">
              • <strong>Opacity Slider:</strong> Adjust window transparency (30%-100%).<br>
    ...
    const sections = [];
    if (body && nav) {
      // Reorder sections to resemble the tab/sub-tab order (and group related
      // topics), so the manual + contents drawer follow the app's layout.
      const ORDER = [
        'Priorities', 'Pawn Cards', 'Pawn Optimiser', 'Xenotype', 'Custom Traits',
        'Shift Planner', 'Armoury', 'Apparel', 'Blueprints', 'Journal', 'Skills Web',
        'Ideology', 'Relations', 'Records', 'Raids', 'Save File Import', 'Share and Import',
        'Mod Awareness', 'Settings', 'Overlay Controls', 'Formulas',
      ];
      const rank = (div) => {
  ...
  renderRecords() {
    ...
    let pawnsList = search ? pawns.filter(p => (p.nickname || p.name || '').toLowerCase().includes(search)) : pawns.slice();
    ...
    if (sort) {
      pawnsList.sort((a, b) => {
        if (sort.field === 'name') {
          const r = (a.nickname || a.name || '').localeCompare(b.nickname || b.name || '');
    ...
    const rows = pawnsList.map(p => {
      ...
      return `<tr><td>${_escapeHtml(p.nickname || p.name || '?')}</td>${cells}</tr>`;
    }).join('');
