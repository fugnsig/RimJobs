...
**Status:** Plan only. Do not implement until this plan is reviewed and accepted.
...
`disableJob.target` and `disableWorkTag.target` are distinct namespaces. The exact non-`None` WorkTag values are:
...
The exact case is part of the contract. `AllWork` remains an exact flag. It is not expanded through app incapability options.
...
```ts
C4EvaluationContext.create({ pawn, definitionSnapshot })
  -> Readonly<{
       pawnId,
       evidence,
       capacities,
...
The factory collects C2 once and resolves C3 once for the pawn in that explicit evaluation batch. It freezes or defensively snapshots all values used by C4 and is then reused for every requested job. It is discarded after the batch. No manual invalidation, module-global memo, render-hash dependency, or second mutable source of truth is introduced.
...
- the exact 20-value WorkTag vocabulary and runtime marker;
...
The test must validate the fixture's schema, uniqueness, exact vocabulary, exact direct/unknown partition, path IDs, completeness combinations, and absence of analogue fields on unknown jobs. Later C4 suites must import this fixture rather than duplicate these runtime truths.
...
- `TraitDef.disabledWorkTypes` emits exact `disableJob` targets;
...
- `mentalBreak`: known true only for non-null state plus `causedByMood=true`; known false for explicit null or a complete non-mood state; otherwise unknown;
...
- active-package filtering as an explicit provider input. A definition outside `meta.modIds` cannot make an effective target-save snapshot complete;
...
- leave the ten audited app abstractions unknown;
- leave custom jobs unknown;
...
2. exact `disableJob` target matching;
3. exact `disableWorkTag` intersection;
4. persistent structural conditional hediff restrictions by joining C2 definition effects to body observations;
5. relevant race age/life-stage entry;
6. C3 structural capacity requirements path by path;
7. only the completeness uncertainty that could change this job's answer.
...
- exact job and WorkTag disables, mismatched namespaces, multiple tags, `AllWork`, structured ambiguity, and opaque ambiguity not parsed;
...
```js
C4EvaluationContext.create(pawn, definitionSnapshot, capacityDefinitions)
AvailabilityResolver.resolve(context, jobId) -> AvailabilityReport
```

The context calls `CapabilityEvidence.collectPawnEvidence()` once and `CapacityResolver.resolvePawnCapacities()` once, snapshots the relevant status/definition data, derives status only where every audited input is known, and returns a read-only object. Tests instrument call counts across multiple job evaluations.
...
| C2 targets | exact WorkType, exact WorkTag, scalar and multi flags, `AllWork`, unknown raw target, structured candidates, no raw parsing in C4 |
...
| Permission targets | exact job, exact tags, namespace mismatch, ambiguity unanimous/non-unanimous, no legacy alias parsing |
...
- the exact audit-derived C2 prerequisites are present;
